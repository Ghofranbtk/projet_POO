                    ...
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                ...
                                .addGap(55, 55, 55)
                                .addComponent(Titre, javax.swing.GroupLayout.DEFAULT_SIZE, 204, Short.MAX_VALUE))
                            .addGroup(jPanel2Layout.createSequentialGroup()
...
*/
    ...
    try  {
        ...
        String sql = "SELECT idab FROM Abonne WHERE idab=?";
        pst = con.prepareStatement(sql);
        pst.setString(1, id);
        ResultSet rs = pst.executeQuery();
        if (rs.next()) {
            JOptionPane.showMessageDialog(null, "L'ID " + id + " existe déjà.");
        } else {
            sql = "INSERT INTO Abonne (idab, nomab) values (?, ?)";
            pst = con.prepareStatement(sql);
            pst.setString(1, id);
            pst.setString(2, nom);
            pst.executeUpdate();
...
/*
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        try  {
            Class.forName("com.mysql.cj.jdbc.Driver");
            con=DriverManager.getConnection( "jdbc:mysql://localhost:3306/bib","root","");
            String sql = "DELETE FROM Abonne WHERE idab = ?";
            pst = con.prepareStatement(sql);
            pst.setString(1, ID.getText());
            pst.setString(2, Titre.getText());
            pst.executeUpdate();
...
*/
    ...
    try  {
        ...
        if (rowsAffected == 0) {
            JOptionPane.showMessageDialog(null, "L'abonné avec l'ID " + id + " n'existe pas dans la base de données.");
        } else {
