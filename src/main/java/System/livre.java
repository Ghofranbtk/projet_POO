...
private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {                                         
    try  {
        ...
        String sqlSelect = "SELECT * FROM livre WHERE idlivre = ?";
        ...
        if (rs.next()) {
            ...
            String sqlInsert = "INSERT INTO livre (idlivre, titre) values (?, ?)";
...
*/
    ...
    try  {
        ...
        if (rowsAffected == 0) {
            JOptionPane.showMessageDialog(null, "Le livre avec l'ID " + id + " et le titre " + titre + " n'existe pas dans la base de données.");
        } else {
